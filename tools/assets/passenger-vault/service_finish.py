...
def main():
    ...
    # Save authored quad topology and neutral materials, before render mutation.
    add_context()
    source=out/'service-finish.blend'
    ...
    files=[asset,source,out/'top.png',out/'oblique.png']
    write_manifest(root,out,files,report)
...
def write_manifest(root,out,files,report):
    manifest={'schema':1,'gate':'source-only flush construction; independent review pending','license':'CC0-1.0, original procedural geometry; no external inputs','blender':bpy.app.version_string,'coordinates':{'origin':'room origin, not centred','blender':'x/32,-game_y/32,height/32','gltf':'x/32,height/32,game_y/32'},'geometry':report,'scope':'zero-thickness planar cover markings and seams only; no pipes, holes, new collision or runtime changes','context':'Source-only flat eight-solid reservation silhouettes below kit, retained in the editable blend and PNG diagrams, excluded from GLB; not equipment meshes. East wall footprint exposed to show channel inside wall. No hidden pipes are asserted.','rebuild':'blender -t 6 --background --factory-startup --python-exit-code 1 --python tools/assets/passenger-vault/service_finish.py -- --output-root ROOT; verify with python3 tools/assets/passenger-vault/verify_service_finish.py','surface_integration':'Future renderer must resolve coplanar floor markings with a decal/merge strategy; no epsilon height or floor cut introduced here.','files':{str(p.relative_to(root)):{'sha256':hashlib.sha256(p.read_bytes()).hexdigest(),'bytes':p.stat().st_size} for p in files},'sources':{p.name:hashlib.sha256(p.read_bytes()).hexdigest() for p in [Path(__file__),HERE/'test_service_finish.py']}}
