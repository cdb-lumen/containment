...
class Contract(unittest.TestCase):
    ...
    def test_editable_source(self):
        import hashlib
        source=ASSET.parents[3]/'tools/assets/passenger-vault/service-finish/service-finish.blend'
        self.assertTrue(source.is_file(), 'editable service-finish.blend missing')
        bpy.ops.wm.open_mainfile(filepath=str(source))
        parts=[o for o in bpy.data.objects if o.name in EXPECTED]
        validate(parts)
        self.assertTrue(all(len(p.vertices)==4 for o in parts for p in o.data.polygons), 'authored editable quads')
        context={o.name for o in bpy.data.objects if o.name.startswith('CONTEXT_')}
        self.assertEqual(context, {'CONTEXT_camera_envelope','CONTEXT_floor_40_1160_40_840'} |
                         {'CONTEXT_reservation_'+n for n in ['A','B','C','D4','SN','SS','MN','MS']})
        self.assertEqual(len(bpy.data.objects),23)
        ...
        for o in sorted(bpy.data.objects,key=lambda o:o.name):
            self.assertEqual(o.type,'MESH')
            snapshot.append({'name':o.name,'matrix':[list(row) for row in o.matrix_world],
                             'vertices':[list(v.co) for v in o.data.vertices],
                             'faces':[(list(p.vertices),p.material_index) for p in o.data.polygons],
                             'materials':[(m.name,list(m.diffuse_color)) for m in o.data.materials]})
            self.assertTrue(all(m.node_tree.nodes.get('Principled BSDF') is not None for m in o.data.materials))
        semantic=hashlib.sha256(json.dumps(snapshot,sort_keys=True).encode()).hexdigest()
        print('SERVICE_FINISH_SOURCE '+json.dumps({'semantic_sha256':semantic,'objects':23,'kit_meshes':13,'context_meshes':10,'editable_quads':True},sort_keys=True))
