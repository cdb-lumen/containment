...
from distribution import reset, mat, box, vertices, bounds, tree, export, load, render
import room_fit
...
def author(variant):
    ...
    for n,y in [('north',1.21),('south',-1.21)]:B('side_'+n,(0,y,(top+.12)/2),(3.75,.08,top-.12))
    ...
    p.append(wedge('instrument_wedge',-.65,1.7,-1.13,1.13,top+.12,h-.04,.04,shell))
    B('rear_service_cover',(1.66,0,(top+.08+h-.08)/2),(.08,2.26,h-.08-(top+.08)))
    # Instrument bed has a shallow slope and maintains the same gauges south.
    slope=(h-.04-(top+.12))/2.35
    z=lambda x:top+.12+(x+.65)*slope
    for name,y0,y1 in [('north',1.09,1.13),('south',-1.13,-1.09)]:
        p.append(wedge('instrument_cheek_'+name,-.65,1.7,y0,y1,z(-.65)-.035,z(1.7)-.035,.04,shell,bottom=top+.075))
    p.append(wedge('display_bezel',-.57,1.52,-1.05,1.05,z(-.57)+.012,z(1.52)+.012,.018,seal))
    p.append(wedge('operating_display',-.46,1.30,-.94,.94,z(-.46)+.025,z(1.30)+.025,.012,inside))
    for i,y in enumerate([-.605,.605]):
        ...
        B(f'electronics_tray_{i}',(.3,y,.16),(2.3,1.04,.08),inside)
        # South uses a lower electronics enclosure, not a transform squash.
        ph=.36 if variant=='north' else .22
        B(f'electronics_pack_{i}',(.3,y,.20+ph/2),(1.6,.8,ph),inside)
...
def validate(parts,variant):
    ...
    for i,y in enumerate([-.605,.605]):
        ...
        a=ray('deck_plinth',(.3,y,2),(0,0,-1));b0=ray(f'electronics_tray_{i}',(.3,y,-1),(0,0,1))
        assert abs(a.z-b0.z)<1e-5,'tray support';contacts+=1
    slope=.16/2.35
    for x in [-.35,.4,1.2]:
        for y in [-.85,0,.85]:
            z0=top+.12+(x+.65)*slope+.025
    ...
    for name,x in [('service_worktop',-.65),('rear_service_cover',1.66)]:
        a=ray(name,(x,0,2),(0,0,-1));b0=ray('instrument_wedge',(x,0,-1),(0,0,1))
        assert abs(a.z-b0.z)<.004,'instrument support';contacts+=1
    return dict(bounds_blender_m=b,mesh_objects=len(parts),triangles=triangles,deck_probes=deck,support_contacts=contacts,exposed_fittings=exposed,closed_front_probes=closed)
...
def main():
    ...
    for variant in HEIGHTS:
        ...
        render(out/f'{variant}-cutaway.png',cut,(-6,-6,4),(0,0,.5),5.8,f'SOURCE ONLY | {variant} cutaway | top / near side / one face hidden')
