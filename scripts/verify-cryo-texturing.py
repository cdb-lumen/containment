...
b,bs=glb(E/'iteration-before/sealed-cryo.glb');a,asize=glb(ROOT/'public/assets/benchmark/sealed-cryo.glb')
assert len(a['materials'])==len(b['materials'])==8
assert len([m for m in a['materials'] if 'normalTexture' in m])==0
assert len([m for m in a['materials'] if 'metallicRoughnessTexture' in m.get('pbrMetallicRoughness',{})])==3
assert len(a['images'])==8
assert asize < 1100000  # retain prior isolated benchmark ceiling
...
result={'status':'PASS','geometryAndVertexNormalsExact':True,'namedMeshes':len(after),'materialsBefore':len(b['materials']),'materialsAfter':len(a['materials']),'bytesBefore':bs,'bytesAfter':asize,'embeddedImages':len(a['images']),'normalMappedMaterials':0,'roughnessMappedMaterials':3,'sourceTextureType':'Localized world-position causal wear atlases, no normal maps','sha256':hashlib.sha256((ROOT/'public/assets/benchmark/sealed-cryo.glb').read_bytes()).hexdigest()}
...
(E/'iteration-verification.json').write_text(json.dumps(result,indent=2));print(json.dumps(result,indent=2))
