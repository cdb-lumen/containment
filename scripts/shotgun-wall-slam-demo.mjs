...
const root=resolve(process.env.DEMO_ROOT||process.cwd()),out=resolve(process.env.DEMO_OUT||'/home/chernodubv/artifacts/shotgun-wall-slam'),label=process.env.DEMO_LABEL||'after';
...
try{
 ...
 const setup=await page.evaluate(async ({quality,north,family})=>{
  ...
  g.pending=[];g.director.update=()=>[];g.switchWeapon('shotgun');g.enemies.reset();g.combat.setBuild({mutations:['breacher','seismic-impact','heavy-pellets']});Object.assign(g.player,{x:500,y:300,angle:-Math.PI/2});
  const spawn=g.enemies.spawn(family,500,172,true);if(!spawn.spawned)throw Error('Spawn rejected');d.targetId=spawn.enemy.id;d.initial=spawn.enemy;
  if(!canOccupyExpedition(g.geometry,spawn.enemy,spawn.enemy.radius))throw Error('Invalid enemy position');
  ...
  r.setQuality(quality);r.loadRoom(g.node);r.render(g,1,false);d.hud();d.syncScreen(true);d.events=[];d.frame=0;d.damage=[];const damage=g.damage.bind(g);g.damage=(id,amount,impulse)=>{const result=damage(id,amount,impulse);d.damage.push({frame:d.frame,id,amount,secondary:impulse===undefined,...result});return result;};
  ...
  return {enemy:d.initial,build:g.combat.build,player:{...g.player},room:g.node.templateId,quality:r.qualityTier,controlled:true,nativeZoom:r.camera.zoom};
 },{quality:process.env.DEMO_QUALITY||'high',north:!!process.env.DEMO_NORTH,family:process.env.DEMO_FAMILY||'spitter'});
 await page.screenshot({path:join(out,`${label}-setup.png`)});console.log('SETUP',JSON.stringify(setup));
 const fps=30,total=process.env.DEMO_PROBE?25:90,mp4=join(out,`${label}.mp4`);
 ...
 for(let frame=0;frame<total;frame++){
  const state=await page.evaluate(({frame,fps,label,north})=>{
   ...
   const oblique=false,angle=-Math.PI/2,fire=frame===6;
   document.querySelector('#metal-caption').textContent=`${label.toUpperCase()}  /  SHOTGUN  /  BREACHER + SEISMIC WALL SLAM  /  CONTROLLED GAMEPLAY`;
   ...
   return {frame,enemy:g.enemies.getSnapshot(d.targetId),bullets:g.bullets.length,counts:r.effects.counts,elapsed:g.elapsed};
  ...
  if(frame>=6&&frame<=25)await writeFile(join(out,`${label}-${frame}.png`),png);
 ...
 assertSource();
 const {events,damage}=await page.evaluate(()=>({events:window.__metalDemo.events,damage:window.__metalDemo.damage}));assert(damage.some(d=>d.secondary&&d.applied&&d.amount>0),'No actual skill damage');assert.deepEqual(errors,[]);assert(samples.every(s=>s.counts&&Number.isFinite(s.counts.glow)));
 const hits=events.filter(e=>e.type==='hit'&&e.weapon==='shotgun');
 assert(events.some(e=>e.contact));
 const slams=events.filter(e=>e.type==='wall-slam'||e.type==='explosion');assert(slams.length>0,'No actual wall collision effect');
 assert(samples.some((s,i)=>i>0&&s.enemy&&samples[i-1].enemy&&s.enemy.y<samples[i-1].enemy.y-2),'No native launch displacement');
 assert(samples.some(s=>s.enemy&&s.enemy.health+s.enemy.armor<setup.enemy.health+setup.enemy.armor),'No damage');
 ...
 await writeFile(join(out,`${label}.json`),JSON.stringify({root,sha,setup,fps,total,events,damage,samples,errors,probe},null,2));console.log('PASS',JSON.stringify({mp4,hits:hits.length,total,errors}));
