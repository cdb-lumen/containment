...
try{
 ...
 const setup=await page.evaluate(async ({quality,north})=>{
  ...
  g.pending=[];g.director.update=()=>[];g.switchWeapon('shotgun');g.enemies.reset();g.combat.setBuild({mutations:['breacher','seismic-impact','heavy-pellets']});Object.assign(g.player,{x:760,y:205,angle:-Math.PI/2});
  const spawn=g.enemies.spawn('brute',760,88,true);if(!spawn.spawned)throw Error('Spawn rejected');d.targetId=spawn.enemy.id;d.initial=spawn.enemy;
 ...
 assert(samples.some(s=>s.enemy&&s.enemy.y<setup.enemy.y-2),'No native launch displacement');
