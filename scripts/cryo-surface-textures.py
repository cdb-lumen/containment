"""Geometry-locked material blocks and localized seal frost, no normal bake.
Three compact per-part colour atlases plus one stainless roughness atlas replace
nine invisible noise maps. The graphics and their UVs remain untouched.
"""
import numpy as np
import bpy, math
...
def surface_textures(asset, out):
 ...
 for prefix,(base,metal,roughness) in settings.items():
  ...
  if prefix=='02':continue
  size=256;grid=math.ceil(math.sqrt(len(parts)));tile=size//grid
  ...
  if prefix=='03':links.new(image('roughness',rough).outputs['Color'],p.inputs['Roughness'])
