...
window.__blockout={
 ...
 snapshot(){const T=EvidenceThree,actorVisibility=[.2,1,2.05].map(height=>{const world=new T.Vector3(game.player.x/32,height,game.player.y/32),p=world.clone().project(renderer.camera),ray=new T.Raycaster();ray.setFromCamera(new T.Vector2(p.x,p.y),renderer.camera);const hit=ray.intersectObject(renderer.world,true)[0];return {height,pixel:[(p.x+1)*innerWidth/2,(1-p.y)*innerHeight/2],occluded:!!hit&&hit.distance<ray.ray.origin.distanceTo(world)-.01};});return {equipmentState:renderer.passengerVault?.state,equipmentError:String(renderer.passengerVault?.error??''),equipmentInstances:renderer.world.getObjectByName('passenger-vault-equipment')?.userData.instances,actorVisibility,state:game.status,room:game.node.templateId,player:{x:game.player.x,y:game.player.y},checks,travel,passengers:renderer.world.getObjectByName('authored-passenger-vault')?.userData.sealedPassengers,
 ...
 for(const [name,viewport,mobile] of [['desktop',{width:1280,height:900},false],['portrait',{width:390,height:844},true]]){
  ...
  try{
   ...
   for(const [view,route] of [...views,['full-room',[]]]){
    ...
    assert.equal(snapshot.equipmentState,'ready',snapshot.equipmentError);assert.equal(snapshot.equipmentInstances,25);assert.equal(snapshot.room,'passenger-vault');assert.equal(snapshot.state,'playing');assert.equal(snapshot.camera.zoom,1);assert.equal(snapshot.quality,'high');assert.equal(snapshot.passengers,16);assert.ok(snapshot.drawCalls>0);assert.equal(snapshot.contextLost,false);assert.deepEqual(snapshot.errors,[]);assert.deepEqual(errors,[]);
