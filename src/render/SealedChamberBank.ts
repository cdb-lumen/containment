...
export function buildSealedBank(source:T.Group){
 ...
 source.traverse(o=>{if(!(o instanceof T.Mesh))return;
  if(o instanceof T.SkinnedMesh||Array.isArray(o.material)||!o.geometry.attributes.normal||!o.geometry.attributes.uv)throw Error('unsupported chamber mesh');
  const mesh=new T.InstancedMesh(o.geometry,o.material,slots.length);mesh.name=o.name;mesh.castShadow=mesh.receiveShadow=true;
 ...
 for(const circuit of [0,1]){
  ...
  for(const {x,z} of slots){const dx=x-490,dz=z-295,end=[(circuit===0?478.48:493.84)+dx,24.42,258.04+dz];
   ...
   for(let i=1;i<points.length;i++){const a=new T.Vector3(...points[i-1]).divideScalar(U),b=new T.Vector3(...points[i]).divideScalar(U),d=b.clone().sub(a);const g=new T.CylinderGeometry(.8/U,.8/U,d.length(),12);g.applyQuaternion(new T.Quaternion().setFromUnitVectors(new T.Vector3(0,1,0),d.normalize()));g.translate(...a.add(b).multiplyScalar(.5).toArray());parts.push(g);}
  }
  const geometry=mergeGeometries(parts,false)!;parts.forEach(p=>p.dispose());const material=new T.MeshStandardMaterial({color:circuit===0?0x557984:0x8e795b,metalness:.65,roughness:.48});
...
export function attachSealedBank(room:T.Group,onChanged:()=>void,load:(signal:AbortSignal)=>Promise<T.Group>=loadSource,timeoutMs=8000){
 ...
 const owner={state:'loading',ready:Promise.resolve(),dispose(){if(!active&&owner.state==='disposed')return;active=false;controller.abort();clearTimeout(timer);if(bank)retire(bank,...source?[source]:[]);owner.state='disposed';}};
 ...
 owner.ready=Promise.race([pending,timeout]).then(s=>{
  ...
  bank=buildSealedBank(s);const racks=createAwakeningRacks(true);racks.userData.serviceRoutes=bank.userData.serviceRoutes;racks.add(bank);room.add(racks);room.userData.serviceRoutes=racks.userData.serviceRoutes;disposeModel(fallback);owner.state='ready';onChanged();
 }).catch(()=>{if(source){retire(...bank?[bank,source]:[source]);source=undefined;bank=undefined;}if(active){active=false;controller.abort();owner.state='fallback';}}).finally(()=>clearTimeout(timer));
 return owner;
}
