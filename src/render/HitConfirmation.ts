...
export class HitConfirmation {
 ...
 confirm(contact:'damage'|'armor'|'blocked'){if(!this.active)return;this.life=.12;this.marker.dataset.contact=contact;this.marker.hidden=false;this.marker.style.opacity='1';}
 update(dt:number,playing:boolean){this.life=playing?Math.max(0,this.life-dt):0;this.marker.hidden=this.life===0;this.marker.style.opacity=String(Math.min(1,this.life/.05));}
