...
export type TemplateId = 'receiving' | 'forklift-loop' | 'cold-aisle' | 'freight-crossing' | 'pump-ring' | 'specimen-bay' | 'security-lock' | 'reactor-vault'|'rail-depot'|'quarantine-cross'|'cryo-gallery'|'filtration'|'turbine-hall'|'waste-processing'|'power-conduits'|'hive-approach';
...
export type RunState = Readonly<{version:1|2;seed:number;currentNodeId:string;completedNodeIds:readonly string[];phase:RunPhase;draftRoll?:number}>;
