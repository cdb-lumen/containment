...
export class ElementalRuntime{
 ...
 hit(shot:string,weapon:BuildWeapon,t:MutationTarget,direction:Point,before:{chill:number;burn:boolean},primed=false){
  ...
  if(primed)this.primed.add(shot);
  const poisoned=this.poisoned(t.id);let thermal=false,combusted=false;
  if(this.has('thermal-shock')&&before.chill&&before.burn&&this.ready(`thermal:${t.id}`,1500)){
   thermal=true;this.h.clearChill(t.id);this.h.clearBurn(t.id);this.blast(t,32,90,4,cause,`${id}:thermal`,'shatter');
  }
  if(this.has('combustion')&&before.burn&&poisoned&&this.ready('combustion',1500)&&this.first(`combustion:${shot}`)){
   combusted=true;this.poison.delete(t.id);this.blast(t,26,85,4,cause,`${id}:combustion`);
  }
  ...
  if(!thermal&&this.has('absolute-zero')&&this.h.stacks(t.id)>=3&&this.ready(`zero:${t.id}`,2000)){if(!t.immovable)this.roots.set(t.id,this.time+350);this.strike(t,18,cause,`${id}:zero`);this.h.effect('shatter',t.x,t.y);}
 ...
 kill(t:MutationTarget,source:'direct'|'secondary',cause:BuildCause){
  ...
  if(source==='direct'&&this.primed.has(cause.chainId)&&this.has('reactor-cascade')&&this.first(`cascade:${cause.chainId}`))this.blast(t,36,115,5,next,`${id}:cascade`,'overload');
