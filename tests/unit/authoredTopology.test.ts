...
const probes=[{x:420,y:280},{x:680,y:380},{x:600,y:440},{x:600,y:280},{x:785,y:395}];
describe('authored walkable topologies',()=>{
 it('uses exactly five shared polygon topologies with real voids',()=>{
  ...
  for(const n of nodes){const t=ROOM_TEMPLATES[n.templateId];if(ids.includes(n.templateId)){
   ...
   for(const radius of [0,9,28,38]){
    expect(canOccupyExpedition(g,n.templateId==='communal-atrium'?{x:-1,y:50}:n.templateId==='passenger-vault'?{x:30,y:50}:{x:50,y:50},radius)).toBe(false);
 ...
 it('blocks sightlines and player/corpse movement into cryo void but keeps cross-aisle open',()=>{
  ...
  for(const radius of [28,38]){const moved=game.moveCorpse({x:420,y:440,radius},0,-200);expect(moved.blocked).toBe(true);expect(moved.y).toBeGreaterThanOrEqual(360+radius);}
 ...
 it('routes real brutes around voids including a clear ray with insufficient body clearance',()=>{
  for(const [start,target] of [[{x:280,y:380},{x:1000,y:380}],[{x:260,y:250},{x:1000,y:280}]]){
 ...
 it('stops actual player and hazard bullets at cryo void edges',()=>{
  for(const kind of ['player','hazard'] as const){const game=new DepthGame();game.geometry=createExpeditionGeometry(nodes.find(n=>n.templateId==='passenger-vault')!);game.navigation=new FacilityNavigation(game.geometry);game.status='playing';Object.assign(game.player,{x:100,y:440});
   game.bullets.push({kind,x:420,y:440,life:2,request:{weaponId:'plasma',damage:20,speed:1000,radius:9,angle:-Math.PI/2,penetration:1,splashRadius:0,knockback:0},tracker:new ProjectileHitTracker(1)});
 ...
 it('never magnets pickups through the corner of a void',()=>{
  const game=new DepthGame();game.geometry=createExpeditionGeometry(nodes.find(n=>n.templateId==='passenger-vault')!);game.navigation=new FacilityNavigation(game.geometry);game.status='playing';Object.assign(game.player,{x:500,y:388});
  expect(game.pickups.spawn('armor',10,548,340).spawned).toBe(true);
